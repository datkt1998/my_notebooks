...
class Item(ItemBase):
    ...
    )  # read the data with data is not only a dict, it can be an atribute of the class
...
class User(
    UserBase
):  # will be used when reading a user (returning it from the API) doesn't include the password.
    id: int
    is_active: bool
    items: list[Item] = []
